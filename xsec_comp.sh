...
xsecs_sofia_json=$(git show nanoAOD_v15:nanoAOD_v12/datasets.json | jq -r ' to_entries | map(select(.value.era == "2023postBPix") | {(.key | split("13p6TeV")[0] + "13p6TeV"): {"xsec_sofia": .value.xsec}}) | add ')
