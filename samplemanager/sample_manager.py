...
class SampleManager(object):
    ...
    def update_xsec(self):
        # Ask if user wants single or multiple sample mode
        mode = questionary.select(
            "Update xsec for one sample or multiple samples?",
            choices=["Search by single nick", "Search by era (multiple selection)"],
            style=custom_style,
        ).ask()
        ...
        if mode == "Search by single nick":
            nick = questionary.autocomplete(
                "Enter a sample nick to search for",
                list(self.database.samplenicks) + list(self.database.dasnicks),
                style=custom_style,
            ).ask()
            ...
            # Resolve DAS nick to sample nick if necessary
            if nick in self.database.dasnicks and nick not in self.database.samplenicks:
                nick = self.database.get_nick_by_das(nick)
            ...
            if selected_eras:
                nicks_by_era = [
                    nick for nick in self.database.samplenicks 
            ...
            nicks = questionary.checkbox(
                "Select sample nicks to update xsec for",
                choices=sorted(nicks_by_era),
                style=custom_style,
            ).ask()
        ...
        # Execute the update for all selected nicks
        questionary.print(f"Updating xsec for {len(nicks)} samples...")
        for nick in nicks:
            self.database.xsec_by_nick(nick)
        
        questionary.print("Finished updating cross sections.")
